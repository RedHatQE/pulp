...
public class ErrataTest extends PulpTestScript {
	...
	public void uploadTestPkgs() {
		for (List<Object> repoPkg : getUploadRepoPkgData()) {
			...
			/*
			while (!servertasks.getMetadataStatus((String)repoPkg.get(0)).contains("finished")) {
				try {
	...
	public void bindAndPublishTestPkgRepo() {
	...
	@BeforeClass (groups={"testErrata"}, dependsOnMethods={"bindAndPublishTestPkgRepo"})
...
*/
	...
	@Test (groups={"testErrata", "consumerOnly"}, dataProvider="errataData", dependsOnMethods={"listAvailErrataByRepo"})
	public void verifyErrataInfo(String repoId, String consumreId, String errataId, PulpTasks task) {
		ArrayList<Hashtable<String, ArrayList<String>>> filteredResult = this.match(task.listErrata(repoId), ":");
		Hashtable<String, ArrayList<String>> result = pulpAbs.findRepo(filteredResult, errataId);
		ArrayList<String> pkgUpdateList = result.get("Filename");

		System.out.println("************************************************************");
		System.out.println(filteredResult);
		System.out.println(result);
		System.out.println(pkgUpdateList);
	...
	public void installErrataConsumer(String repoId, String consumerId, String errataId, PulpTasks task) {
		...
		Assert.assertTrue(result.contains("Install Succeeded"), "Verifying pulp reports errata installation was successful.");
	}
...
*/
	...
	public List<List<Object>> getLocalRepoData() {
		...
		repoOpts.add("--repo-id=errata_test_repo");
		...
		repoOpts.add("--repo-id=errata_pkg_upload_repo");
		data.add(Arrays.asList(new Object[]{repoOpts}));
		return data;
	}
