package com.redhat.qe.pulp.v2_cli.tasks;
...
public class PulpTasks {
	...
	public PulpTasks(SSHCommandRunner runner) {
		...
		try {
			...
			//groovyClass = loader.parseClass(new File(System.getProperty("automation.dir")+"/src/com/redhat/qe/pulp/cli/base/YumMetadataParser.groovy"));
	...
	public String createConsumer(String consumerId, boolean expectSuccess) {
		if (expectSuccess) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-consumer -u admin -p admin register --consumer-id "+consumerId).getExitCode(), Integer.valueOf(0), "Creating consumer: ");
		}
		else {
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait("pulp-consumer -u admin -p admin register --consumer-id "+consumerId).getExitCode(), Integer.valueOf(0), "Creating consumer: ");
		}
	...
	public String bindConsumer(String consumerId, String repoId, boolean expectSuccess) {
		if (expectSuccess) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-consumer bind --repo-id "+repoId).getExitCode(), Integer.valueOf(0), "Binding consumer to repo: ");
		}
		else {
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait("pulp-consumer bind --repo-id "+repoId).getExitCode(), Integer.valueOf(0), "Binding consumer to repo: ");
		}
	...
	public String clientDeleteConsumer(boolean expectSuccess) {
		if (expectSuccess) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-consumer unregister").getExitCode(), Integer.valueOf(0), "Deleting consumer: ");
		}
		else {
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait("pulp-consumer unregister").getExitCode(), Integer.valueOf(0), "Deleting consumer: ");
		}
		return sshCommandRunner.getStderr();
	}
	...
	public void clientUnbindConsumer(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-consumer unbind --repo-id "+repoId).getExitCode(), Integer.valueOf(0), "Unbinding consumer from repo");
	}

	public void unbindConsumer(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-consumer unbind --repo-id "+repoId).getExitCode(), Integer.valueOf(0), "Unbinding consumer from repo");
	}


	public void login() {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin login -u admin -p admin").getExitCode(), Integer.valueOf(0), "Loging in: ");
	}

	public void logout() {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin logout").getExitCode(), Integer.valueOf(0), "Loging out: ");
	}
	...
	public String listConsumer() {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-consumer status").getExitCode(), Integer.valueOf(0));
		return sshCommandRunner.getStdout();
	}
	...
	public String installPackage(String pkgName, String consumerId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm consumer package install run -n "+pkgName+" --consumer-id=" + consumerId).getExitCode(), Integer.valueOf(0), "Installing pkg on consumer "+consumerId);
		return sshCommandRunner.getStdout();
	}

	public String installPkgGroup(String pkgGroupName, String consumerId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin consumer package-group install --consumer-id="+consumerId+" --name="+pkgGroupName).getExitCode(), Integer.valueOf(0), "Installing "+pkgGroupName);
		return sshCommandRunner.getStdout();
	}

	public void updatePkgInfo() {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-consumer update").getExitCode(), Integer.valueOf(0), "Update consumer pkg profile.");
	}

	public String listErrataByConsumer(String consumerId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content errata --consumerid="+consumerId).getExitCode(), Integer.valueOf(0), "Listing errata for "+consumerId);
		return sshCommandRunner.getStdout();
	}

	public String listErrataByRepo(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content errata --repo-id="+repoId + " --match='name=*'").getExitCode(), Integer.valueOf(0), "Listing errata for "+repoId);
		return sshCommandRunner.getStdout();
	}

	public String listErrata(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content errata --repo-id="+repoId).getExitCode(), Integer.valueOf(0), "");
		return sshCommandRunner.getStdout();
	}

	public String errataInfo(String repoId, String errataId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content errata --repo-id="+repoId + " --match='name=" + errataId + "'").getExitCode(), Integer.valueOf(0), "Errata info for "+errataId);
		return sshCommandRunner.getStdout();
	}
	
	public String installErrata(String consumerId, String errataId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm consumer errata install run --consumer-id="+consumerId+" -e "+errataId).getExitCode(), Integer.valueOf(0), "Installing "+errataId);
		return sshCommandRunner.getStdout();
	}
	...
	public String listRepoStatus(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo sync status --repo-id="+repoId).getExitCode(), Integer.valueOf(0));
		return sshCommandRunner.getStdout();
	}
	...
	public String syncTestRepo(String repoId, boolean follow) {
		...
		if (follow) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo sync run --repo-id "+repoId).getExitCode(), Integer.valueOf(0), "Syncing Repo");
		}
		else {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo sync run --repo-id "+repoId + " --bg").getExitCode(), Integer.valueOf(0), "Syncing Repo");
		}
		return sshCommandRunner.getStdout();
	}

	public void createTestRepo(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo create --repo-id=" + repoId).getExitCode(), Integer.valueOf(0), "Creating repo: ");
	}

	public void createTestRepo(ArrayList fields) {
		StringBuffer cmd = new StringBuffer("pulp-admin rpm repo create ");
	...
	public void createTestRepoWithMetadata(ArrayList fields) {
		StringBuffer cmd = new StringBuffer("pulp-admin rpm repo create --preserve_metadata ");
		Iterator itr = fields.iterator();
		while (itr.hasNext()) {
			cmd.append((String)itr.next()).append(" ");
		}
	...
	public void createTestRepoWithMetadata(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo create --repo-id="+repoId+" --name="+repoId + " --preserve_metadata").getExitCode(), Integer.valueOf(0), "Creating repo: ");
	}

	public void createTestRepo(String repoId, boolean expectedSuccess) {
		if (expectedSuccess) {
			createTestRepo(repoId);
		} else {
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo create --repo-id=" + repoId).getExitCode(), Integer.valueOf(0), "Creating repo: ");
		}
	}

	public void createTestRepo(ArrayList fields, boolean expectedSuccess) {
		if (expectedSuccess) {
			createTestRepo(fields);
		} else {
			StringBuffer cmd = new StringBuffer("pulp-admin rpm repo create ");
			Iterator itr = fields.iterator();
			while (itr.hasNext()) {
			...
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait(cmd.toString()).getExitCode(), Integer.valueOf(0), "Creating repo: ");
		}
	}

	public void cloneTestRepo(ArrayList<String> fields) {
		StringBuffer cmd = new StringBuffer("pulp-admin rpm repo copy ");
		Iterator itr = fields.iterator();
		while (itr.hasNext()) {
			cmd.append((String)itr.next()).append(" ");
		}
	...
	public String deleteRepo(String repoId) {
		sshCommandRunner.runCommandAndWait("pulp-admin rpm repo delete --repo-id="+repoId);
	...
	public String listRepo(boolean expectSuccess) {
		String rtn = "";
		if (expectSuccess) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo list --details").getExitCode(), Integer.valueOf(0));
		}
		else {
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo list --details").getExitCode(), Integer.valueOf(0));
		}
		rtn = sshCommandRunner.getStdout() + sshCommandRunner.getStderr();
		return rtn;
	}
	...
	public String getRepoPackageList(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content rpm --repo-id="+repoId + " --match='name=.*'").getExitCode(), Integer.valueOf(0));
		return sshCommandRunner.getStdout();
	}
	...
	public void createRelativeTestRepo(String repoLocation, String repoId, String repoArch, String relativePath, String repoSchedule) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo create --repo-id="+repoId+" --name="+repoId+" --arch="+repoArch+" --feed="+repoLocation+" --schedule=\""+repoSchedule+"\""+" --relativepath="+relativePath).getExitCode(), Integer.valueOf(0), "Creating relative repo: ");
	}

	public void createCertTestRepo(String repoLocation, String repoId, String repoArch, String certPath) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo create --repo-id="+repoId+" --name="+repoId+" --arch="+repoArch+" --feed="+repoLocation).getExitCode()+" --cacert="+certPath, Integer.valueOf(0), "Creating certificate repo: ");
	}

	public void cancelSyncTestRepo(String taskId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin tasks cancel --task-id="+taskId).getExitCode(), Integer.valueOf(0), "Unsyncing repo: ");
	}
...
*/
	...
	public String updateTestRepo(ArrayList<String> fields, boolean expectSuccess) {
		StringBuffer cmd = new StringBuffer("pulp-admin rpm repo update ");
		Iterator itr = fields.iterator();
		while (itr.hasNext()) {
			cmd.append((String)itr.next()).append(" ");
		}
	...
	public String uploadContent(String repoId, String rpmName, String rpmUrl, String tmpRpmDir, boolean expectSuccess) {
		...
		if (expectSuccess) {
			if (repoId.equals("")) {
				Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo uploads rpm -v --file "+tmpRpmDir+"/"+rpmName).getExitCode(), Integer.valueOf(0), "Uploading package to pulp without association to repo: ");
			}
			else {
				Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo uploads rpm -v --repo-id="+repoId+" --file "+tmpRpmDir+"/"+rpmName).getExitCode(), Integer.valueOf(0), "Uploading package to repo: ");
			}
		}
		else {
			if (repoId.equals("")) {
				...
				sshCommandRunner.runCommandAndWait("pulp-admin rpm repo uploads rpm -v "+tmpRpmDir+"/"+rpmName);
			}
			else {
				...
				// Temporary workaround for content bz 752098
				sshCommandRunner.runCommandAndWait("pulp-admin rpm repo uploads rpm -v --repo-id="+repoId+" --file "+tmpRpmDir+"/"+rpmName);
			}
		}
		return sshCommandRunner.getStdout();
	}
	...
	public String uploadChunkedContent(String fileName, String fileUrl, String tmpDir, String chunkSize, SSHCommandRunner subRunner) {
		...
		sshCommandRunner.setCommand("pulp-admin rpm repo upload --chunksize="+chunkSize + " " +tmpDir+"/"+fileName);
	...
	public void deleteRPM(String repoId, String rpmName) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo delete_package --id="+repoId+" -p "+rpmName).getExitCode(), Integer.valueOf(0), "Deleting package from repo: ");
	}
	...
	public void packageList(String repoId, String queryName) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content rpm --repo-id="+repoId+" --match='name="+queryName + "'").getExitCode(), Integer.valueOf(0), "Checking pkg info");
	}

	public void installGroupPackage(String pkgName, String consumerGroupId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin consumer rpm consumergroup package install run -n "+pkgName+" --consumergroupid=" + consumerGroupId, Long.valueOf(60000)).getExitCode(), Integer.valueOf(0), "Installing pkg on consumer "+consumerGroupId);
	}
	...
	public void createPkgGroup(String pkgGroupName, String repoId, ArrayList<String> pkgList) {
		StringBuffer cmd = new StringBuffer("pulp-admin repo uploads group --group-id=" + pkgGroupName + " --repo-id=" + repoId + " --name=" + pkgGroupName + " --description=" + pkgGroupName + " ");
		for (String pkg : pkgList) {
			cmd.append(" --default-name=" + pkg);
		}

		Assert.assertEquals(sshCommandRunner.runCommandAndWait(cmd.toString()).getExitCode(), Integer.valueOf(0), "Creating pkg group ");
	}

/*
	public void addToPkgGroup(String pkgGroupName, String repoId, String pkgName) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin packagegroup add_package --id "+pkgGroupName+" --repo-id="+repoId+" --name="+pkgName).getExitCode(), Integer.valueOf(0), "Adding "+pkgName+" to "+pkgGroupName);
	}

	public void removePkgFromGroup(String pkgGroupName, String repoId, String pkgName) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin packagegroup delete_package --id "+pkgGroupName+" --repo-id="+repoId+" --name="+pkgName).getExitCode(), Integer.valueOf(0), "Removing "+pkgName+" from "+pkgGroupName);
	}
*/

	public String pkgGroupInfo(String pkgGroupName, String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content group --match='name="+pkgGroupName+"' --repo-id="+repoId).getExitCode(), Integer.valueOf(0), "Fetching info on "+pkgGroupName);
		return sshCommandRunner.getStdout();
	}

	public String listPkgGroup(String repoId, boolean expectSuccess) {
		if (expectSuccess) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content group --repo-id=" + repoId).getExitCode(), Integer.valueOf(0), "Listing package groups on "+repoId);
		}
		else {
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo group list --match='name='"+repoId + "'").getExitCode(), Integer.valueOf(0), "Listing package groups on "+repoId);
		}
		return sshCommandRunner.getStdout();
	}
	...
	public String deletePkgGroup(String pkgGroupName, String repoId, boolean expectSuccess) {
		if (expectSuccess) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin repo remove package-group --repo-id="+repoId + " --match='name=" + pkgGroupName + "'").getExitCode(), Integer.valueOf(0), "Deleting "+pkgGroupName);
		}
		else {
			Assert.assertNotSame(sshCommandRunner.runCommandAndWait("pulp-admin repo remove package-group --repo-id="+repoId + " --match='name=" + pkgGroupName + "'").getExitCode(), Integer.valueOf(0), "Deleting "+pkgGroupName);
		}
		return sshCommandRunner.getStdout();
	}

	public String listCategory(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content category --repo-id=" + repoId).getExitCode(), Integer.valueOf(0), "Listing package category on "+repoId);
		return sshCommandRunner.getStdout();
	}

	public String listDistribution(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo content distribution --repo-id=" + repoId).getExitCode(), Integer.valueOf(0), "Listing package distribution on "+repoId);
		return sshCommandRunner.getStdout();
	}
	...
	public String getContentList(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin content list --repo-id="+repoId).getExitCode(), Integer.valueOf(0), "Fetching content list.");
		return sshCommandRunner.getStdout();
	}

	public void deleteOrphanedContent(String fileName) {
		if (fileName.equals("")) {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin orphan remove --all").getExitCode(), Integer.valueOf(0), "Delete all orphan file.");
		}
		else {
			Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin orphan remove --unit-id " + fileName).getExitCode(), Integer.valueOf(0), "Delete orphan file.");
		}
	}
...
/*
	...
	*/
	...
	public void importPkgGroupViaComps(String repoId, String fileName, String fileURL, String tmpDir) {
		fetchFile(fileURL, tmpDir);
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin packagegroup import --repo-id="+repoId+" --comps="+tmpDir+"/"+fileName).getExitCode(), Integer.valueOf(0), "Importing pkg group");
	}

	public void exportPkgGroupViaComps(String repoId, String fileName, String tmpDir) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin packagegroup export --repo-id="+repoId+" --out="+tmpDir+"/"+fileName).getExitCode(), Integer.valueOf(0), "Exporting pkg group");
	}
	...
	public void associateRepoToCDS(String hostname, String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin cds associate_repo --hostname " + hostname + " --repo-id " + repoId).getExitCode(), Integer.valueOf(0), "Associating repo " + repoId + " to "  + hostname + " to CDS.");
	}

	public void unassociateRepoToCDS(String hostname, String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin cds unassociate_repo --hostname " + hostname + " --repo-id " + repoId).getExitCode(), Integer.valueOf(0), "Unassociating repo " + repoId + " to "  + hostname + " to CDS.");
	}
	...
	public String execCmd(String cmd) {
		sshCommandRunner.runCommandAndWait(cmd);
		return (sshCommandRunner.getStdout() + sshCommandRunner.getStderr());
	}
...
/*
	...
	public String getMetadataStatus(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin repo generate_metadata --status --id=" + repoId).getExitCode(), Integer.valueOf(0), "Getting repo metadata status.");
		return sshCommandRunner.getStdout();
	}
*/

	public void publishRepo(String repoId) {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin rpm repo publish run --repo-id=" + repoId).getExitCode(), Integer.valueOf(0), "Publishing repo " + repoId);
	}

	public String getTaskList() {
		Assert.assertEquals(sshCommandRunner.runCommandAndWait("pulp-admin tasks list").getExitCode(), Integer.valueOf(0));
		return (sshCommandRunner.getStdout() + sshCommandRunner.getStderr());
	}
}
