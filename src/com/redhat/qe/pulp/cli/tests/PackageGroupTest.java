...
public class PackageGroupTest extends PulpTestScript {
	...
	public PackageGroupTest() {
		...
		rules.put("pkgGroupMandatoryPkg", "^Mandatory packages[\\s]+([a-zA-Z0-9\\_\\-\\[\\]\\'\\,]+)[\\s]*$");
		rules.put("pkgGroupDefaultPkg", "^Default packages[\\s]+([a-zA-Z0-9\\_\\-\\[\\]\\'\\,\\s]+)[\\s]*$");
		rules.put("pkgGroupOptionalPkg", "^Optional packages[\\s]+([a-zA-Z0-9\\_\\-\\[\\]\\'\\,]+)[\\s]*$");
		rules.put("pkgGroupConditionalPkg", "^Conditional packages[\\s]+([a-zA-Z0-9\\_\\-\\[\\]\\'\\,]+)[\\s]*$");
	...
	public void createPkgGroup(String pkgGroupName, String repoId) {
		servertasks.createPkgGroup(pkgGroupName, repoId);
	}
	...
	public void verifyInstallPkgGroup(String pkgGroupName, String repoId, String consumerId, PulpTasks task) {
		ArrayList<Hashtable<String, ArrayList<String>>> filteredResult = this.match(servertasks.pkgGroupInfo(pkgGroupName, repoId), "    ");
		String[] pkgs = decodePkg(filteredResult.get(0).get("Default packages").get(0));
	...
	@Test (groups={"testPackageGroup"}, dataProvider="pkgGroupData", dependsOnMethods={"removePkgFromGroup"}, alwaysRun=true)
	...
	public void verifyExportPkgGroup(String fileName, String tmpDir) {
		servertasks.localFetchFile(System.getProperty("automation.resources.location") + "/test_comps.xml", "/tmp");
	...
	public List<List<Object>> getLocalRepoData() {
		...
		repoOpts.add("--id=package_group_test_repo");
	...
	public List<List<Object>> getLocalPkgData() {
		ArrayList<List<Object>> data = new ArrayList<List<Object>>();
		data.add(Arrays.asList(new Object[]{"patb-0.1-1.noarch.rpm", auto_resources + "/patb-0.1-1.noarch.rpm", tmpRpmDir, "patb"}));
		data.add(Arrays.asList(new Object[]{"emoticons-0.1-1.noarch.rpm", auto_resources + "/emoticons-0.1-1.noarch.rpm", tmpRpmDir, "emoticons"}));
		data.add(Arrays.asList(new Object[]{"origin-1.0-1.noarch.rpm", auto_resources + "/updates/origin-1.0-1.noarch.rpm", tmpRpmDir, "origin"}));
		data.add(Arrays.asList(new Object[]{"parent-1.0-1.noarch.rpm", auto_resources + "/updates/parent-1.0-1.noarch.rpm", tmpRpmDir, "parent"}));
		data.add(Arrays.asList(new Object[]{"feedless-1.0-1.noarch.rpm", auto_resources + "/updates/feedless-1.0-1.noarch.rpm", tmpRpmDir, "feedless"}));
		return data;
	}
	...
	public List<List<Object>> getCompsFileData() {
		ArrayList<List<Object>> data = new ArrayList<List<Object>>();
		data.add(Arrays.asList(new Object[]{"test_comps.xml", auto_resources + "/test_comps.xml", tmpRpmDir}));
		return data;
	}
